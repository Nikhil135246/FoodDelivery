...
const MainList: FC = () => {
    ...
    const [isNearEnd, setIsNearEnd] = useState(false) // Track if the user is near the end of the list
    ...
    const handleScroll = (event: NativeSyntheticEvent<NativeScrollEvent>) => {
        ...
        const containerHeightt = event.nativeEvent.contentSize.height;
        const layoutHeight = event?.nativeEvent?.layoutMeasurement?.height;
        const Offset = event?.nativeEvent?.contentOffset?.y;
        setIsNearEnd(
            Offset + layoutHeight >= containerHeightt - 500);
                    ...
                    return (
                        <Animated.View
                            style={[
                                ...
                                // Keep the condition but only apply transparent background
